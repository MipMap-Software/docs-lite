...
Create a new project and select LiDAR.

![](/img/en-img/newproject3.png)

### Import images and point clouds

![](/img/en-img/importimg.png)
...
Select point cloud files to import into the current task.

**Import point cloud folder:**

Select a folder to import all point cloud files within it into the current task.

**Import preset data:**

Click ![](/img/en-img/predata.png) and choose an MPL file to quickly import all preset data.

### Edit camera

The software automatically parses the imported photos' file structure and metadata, grouping photos by camera based on parameters found in the photos (focal length, sensor size, intrinsics, distortion, etc.).

If camera parameters cannot be parsed from the photos or are incorrect, you need to edit the camera parameters.

>It is recommended to select camera parameters for your device model from the database.
...
- Get from database: select camera parameters from the built-in database; if the model isn't listed, it cannot be selected.
...
>Note: The LAS point cloud and the image POS must be in the same coordinate system.
...
- Coordinate accuracy: Select the accuracy level for the POS information. Choose according to the actual acquisition device; LiDAR defaults to High Accuracy POS.

![](/img/en-img/posaccuracy.png)

- Elevation offset: Enter an elevation offset value; all elevations in the current POS information will be shifted by this value.

- Export POS: Export the current POS information to a specified folder in CSV format.

### Delete images/point clouds

If imported images or point clouds are redundant or not required for reconstruction, you can delete them.

#### Delete from list

![](/img/en-img/editlas.png)

1. Delete point cloud files or images belonging to a specific camera.

2. Delete all point clouds or images in the task.

#### Delete by selection

![](/img/en-img/editlas2.png)
